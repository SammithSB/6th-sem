...
VAR: T_ID '=' EXPR 	{
     ...
     | T_ID 		{ if(check_symbol_table($1)==1){yyerror("variable redeclared");}
	 				  else{

						   s=allocate_space_for_table_entry($1,size,type,yylineno,1);
...
TYPE : T_INT {type=INT; size=sizeof(int);}
       | T_FLOAT {type=FLOAT;size=sizeof(float);}
       | T_DOUBLE {type=DOUBLE;size=sizeof(double);}
       | T_CHAR {type=CHAR;size=sizeof(char);}
       ;
